...
class Ros2DataModelUtil(DataModelUtil):
    ...
    def get_callback_symbols(self) -> Mapping[int, str]:
        ...
        # Filter out symbols that would cause KeyError
        valid_objects = {obj for obj in callback_objects if obj in callback_symbols.index}

        # Get their symbol
        return {
            obj: self._prettify(callback_symbols.loc[obj, 'symbol']) for obj in valid_objects
        }
