...
path = os.path.expanduser('~/.ros/tracing/callback_function')
...
def get_timer_callback_ranges(timer_node_name: str, timer_callback_name: str) -> List[Tuple[pd.Timestamp, pd.Timestamp, pd.Timedelta]]:
    # Get timer object
    objs_and_owners = {
        obj: data_util.get_callback_owner_info(obj)
        for obj, callback_name in callback_symbols.items()
        if timer_callback_name in callback_name
    }
...
ranges_main_task = get_timer_callback_ranges('robot_status', 'main_task')
...
search_string = 'timer_counter'
num_pos_timer_counter = None
for index, item in enumerate(segment_types):
    if search_string in item:
        num_pos_timer_counter = index
        break
else:
    print(f"'{search_string}' not found in the list")


search_string = 'main_task'
num_pos_main_task = None
for index, item in enumerate(segment_types):
    if search_string in item:
        num_pos_main_task = index
        break
else:
    print(f"'{search_string}' not found in the list")

add_durations_to_figure(fig, 'callback timer_counter', ranges_timer_counter, 'red', line_width, num_pos_timer_counter)
add_durations_to_figure(fig, 'callback main_task', ranges_main_task, 'blue', line_width, num_pos_main_task)
